<script lang="ts">
  ...
  let userId = $me; // como pego esse user?
  let comments = [];
  ...
  async function deleteComment(commentId) {
    try {
      // Chame a sua função de exclusão de comentário da API aqui
      // Substitua pelo caminho real da sua API e ajuste conforme necessário
      const res = await api.delete(`/comments/${commentId}`);
      console.log(res.data); // Apenas para depuração
      // Atualize os comentários após a exclusão
